...
public class RevenueAnalyticsService {
    ...
    public RevenueAnalyticsResponse getRevenueAnalytics() {
        ...
        double totalRevenue =
                getAmount(PaymentStatus.SUCCESS)
                        + getAmount(PaymentStatus.FAILED)
                        + getAmount(PaymentStatus.RETRYING)
                        + getAmount(PaymentStatus.CREATED)
                        + getAmount(PaymentStatus.RECOVERED);
        ...
        if (totalRevenue > 0) {
            ...
            recoveryRate =
                    ((successfulRevenue + recoveredRevenue)
                            / totalRevenue) * 100.0;
        }
    ...
    private double getAmount(
        ...
        Double amount =
                paymentRepository.getTotalAmountByStatus(
                        status
                );
